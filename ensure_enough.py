...
class StateManagement:
    ...
    def non_conflicting_name(prefix, existing_servers):
        """Generate a name for the machine that's unique to the machine and not
        used by any existing ones.

        :param str prefix: the name prefix, usually
            vgcnbwc-{resource_identifier}
        :param list(Nova) existing_servers: list of existing servers against
            which we will check the names.
        """
    ...
    def identify_server_group(self, server_identifier):
        ...
        :param str server_identifier: A string that prefixes all servers of
            that group, e.g. `vgcnbwc-training...`
        ...
        for server in sorted(
            self.os_command(["server", "list"]), key=lambda x: x["Name"]
        ):
            # Filter by those with our server_identifier.
            if server["Name"].startswith(server_identifier):
                server_image_name = server["Image"]
                # if the image isn't the latest / current version, OR if the
                # server isn't running
                if (
                    self.config["image_replace"]
                    and server_image_name != self.current_image_name
    ...
    def wait_for_state(
        self, server_name, target_state, escape_states=None, timeout=600
    ):
        """
        Wait for a server to reach a specific state.

        :param str server_name: Name of the server
        :param str target_state: one of
            https://docs.openstack.org/nova/latest/reference/vm-states.html
        :param list or None escape_states: A list of status that also trigger
            an exit without hitting the timeout.
        ...
        while True:
            ...
            current_servers = {
                x["Name"]: x for x in self.os_command(["server", "list"])
            }
    ...
    def template_config(
        ...
        custom_userdata = re.sub(
            "  GalaxyTraining.*",
            "  GalaxyTraining = %s" % is_training,
            custom_userdata,
        )
        custom_userdata = re.sub(
            "  GalaxyGroup.*", '  GalaxyGroup = "%s"' % group, custom_userdata
        )
        custom_userdata = re.sub(
            "  GalaxyCluster.*", '  GalaxyCluster = "denbi"', custom_userdata
        )
        custom_userdata = re.sub(
            "  GalaxyDockerHack.*",
            "  GalaxyDockerHack = %s" % docker_ready,
            custom_userdata,
        )

        if cgroups:
            custom_userdata = re.sub(
                "# BASE_CGROUP", "BASE_CGROUP", custom_userdata
            )
            policy = cgroups_args.get("mem_limit_policy", None)
            if policy:
                custom_userdata = re.sub(
                    "# CGROUP_MEMORY_LIMIT_POLICY.*",
                    "CGROUP_MEMORY_LIMIT_POLICY = {}".format(policy),
                    custom_userdata,
                )
            size = cgroups_args.get("mem_reserved_size", 1024)
            custom_userdata = re.sub(
                "# RESERVED_MEMORY.*",
                "RESERVED_MEMORY = {}".format(size),
                custom_userdata,
            )

        if gpu_ready:
            custom_userdata = re.sub(
                "# packages:", "packages:", custom_userdata
            )
            custom_userdata = re.sub(
                "# - cuda-10-1", " - cuda-10-1", custom_userdata
            )
            custom_userdata = re.sub(
            ...
            custom_userdata = re.sub(
                "# use feature : GPUs", "use feature : GPUs", custom_userdata
            )
            custom_userdata = re.sub(
    ...
    def _select_image(
        self, gpu_ready=False, secure_ready=False, alma_ready=False
    ):
        if gpu_ready:
    ...
    def launch_server(
        ...
        current_image_name = self._select_image(
            gpu_ready, secure_ready, alma_ready
        )
        ...
        custom_userdata = self.template_config(
            group,
    ...
    def launch_server_volume(
        ...
        args = [
            ...
            "nova",
            "--nic",
            "net-id=%s" % self.config["network_id"],
            "--user-data",
            f.name,
        ]
        for sg in self.config["secgroups"]:
        ...
        if vol_boot:
            ...
            args.append(
                ...
                "volume_size={},"
                "volume_type={},"
                "delete_on_termination=true".format(vol_size, vol_type)
            )
    ...
    def brutally_terminate(self, server):
        ...
        logging.info(
            self.os_command(
                ["server", "delete", server.get("ID", server["Name"])],
                is_json=False,
            )
        )
    ...
    def gracefully_terminate(self, server, patience=300):
        ...
        if server["Status"] == "ACTIVE":
            ...
            while True:
                ...
                if time_slept > patience:
                    logging.info(
                        "%s is busy, giving up for this hour.", server["Name"]
                    )
                    # exit early
                    return
                ...
                stdout, stderr = self.remote_command(
                    ip, "condor_drain `hostname -f`"
                )
                ...
                if "Sent request to drain" in stdout:
                    ...
                    logging.warn(
                        "Something might be wrong: %s, %s", stdout, stderr
                    )
                    break
                ...
                try:
                    ...
                    stdout, stderr = self.remote_command(
                        ip, "condor_status | grep slot.*@`hostname -f`"
                    )
                    condor_statuses = [
                        x.split()[4] for x in stdout.strip().split("\n")
                    ]
                except IndexError:
                    break

                logging.info("condor_status %s", condor_statuses)
                # if 'Retiring' then we're still draining. If 'Idle' then safe
                # to exit.
                if len(condor_statuses) > 1:
                    # The machine is currently busy but will not accept any new
                    # jobs. For now, leave it alone.
                    logging.info("%s is busy, sleeping.", server["Name"])
                    continue
                else:
                    # Ensure we are promptly removed from the pool
                    stdout, stderr = self.remote_command(
                        ip, "/usr/sbin/condor_off -graceful `hostname -f`"
                    )
                    logging.info("/usr/sbin/condor_off %s %s", stdout, stderr)

        # The image is completely drained so we're safe to kill.
        logging.info(
            self.os_command(["server", "delete", server["ID"]], is_json=False)
        )
        ...
        while True:
            # Get the latest listing of servers
            current_servers = [
                x["Name"] for x in self.os_command(["server", "list"])
            ]
    ...
    def top_up(
        ...
        """
        :param int desired_instances: Number of instances of this type to
            launch

        :param str prefix: Something like `vgcnbwc-{resource_id}`

        :param str resource_identifier: Just the `{resource_id}` from previous
            part

        :param str flavor: flavor to launch

        :param str group: The group that it is launched in (compute, upload,
        ...
        # Because we care not about how many are currently ok, but the number
        # of ACTIVE servers that can be processing jobs.
        ...
        for i in range(to_add):
            ...
            if server["Status"] == "ERROR":
                if "ID" in server:
                    fault = self.os_command(
                        ["server", "show", server["ID"]]
                    ).get("fault", {"message": "<error>"})
                else:
                    fault = {"message": "Unknown"}

                logging.error(
                    "Failed to launch %s: %s", server["Name"], fault["message"]
                )
                self.brutally_terminate(server)

                if "There are not enough hosts available" in fault["message"]:
                    logging.warning(
                        "Skipping launch attempt for remaining machines due"
                        "to too-few-hosts error."
                    )
                    break
            else:
                logging.info(
                    "Launched. %s (state=%s)", server, server["Status"]
                )
    ...
    def syncronize_infrastructure(self):
        ...
        for resource_identifier in self.config["deployment"]:
            ...
            # If we have more servers allocated than desired, we should remove
            # some.
            ...
            # If the resource has a `start` or `end` and we are not within that
            # range, then we should move all resources from `servers_ok` to
            # `servers_rm`
            ...
            logging.info(
                "Found %s/%s running, %s to remove",
                len(servers_ok),
                desired_instances,
                len(servers_rm),
            )

            # Ok, here we possibly have some that need to be removed, and
            # possibly have some number that need to be added (of the new image
            # version)

            # We don't want to abuse resources and we'd like to keep within the
            # limited number of VMs to make this more reusable. If we say "max
            # 10 VMs" we should honor that.

            # We will start expiring old ones, "topping up" as we go along.
            for server in servers_rm:
                # we need to SSH in and condor_drain, wait for queue to empty,
                # and then kill.

                # Galaxy-net must be the used network, maybe this check is
                # extraneous but better to only work on things we know are safe
                # to work on.

                # This changed formats, i guess due to versions of
                ...
                if isinstance(server["Networks"], dict):
                    ...
                    netz = {
                        x.split("=")[0]: x.split("=")[1]
                        for x in server["Networks"].split(",")
                    }
                ...
                if self.config["network"] not in netz.keys():
                    ...
                    logging.warn(
                        "Not sure how to handle server %s", server["Name"]
                    )
                    continue

                # Gracefully (or violently, depending on patience) terminate
                # the VM.
            ...
            # Now that we've removed all that we need to remove, again, try to
            # top-up to make sure we're OK. (Also important in case we had no
...
def make_parser():
    parser = argparse.ArgumentParser(
        prog="ensure_enough", description="VGCN Infrastructure Management"
    )
    parser.add_argument(
        "-r",
        "--resources_file",
    ...
    parser.add_argument(
        "-d", "--dry_run", action="store_true", help="dry run mode"
    )
